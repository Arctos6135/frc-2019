...
/**
 * Runs Essie to intake cargo until the sensor inside it is activated. Note that
 * if there is no cargo, this command will never terminate.
 */
public class AutoCargoIntake extends Command {
	...
	protected boolean isFinished() {
		...
		if (Robot.essie.hasCargo()) {
			OI.pickupRumbleOperator_uwu.execute();
			OI.pickupRumbleDriver_uwu.execute();
			@SuppressWarnings("resource")
			Command pulse = new PulseBeautifulRobot(1.5, 10,
					BeautifulRobotDriver.Color.fromAlliance(DriverStation.getInstance().getAlliance()));
			pulse.start();
