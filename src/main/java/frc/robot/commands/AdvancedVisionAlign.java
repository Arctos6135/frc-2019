...
public class AdvancedVisionAlign extends Command {
	...
	protected void initialize() {
		...
		if (!Robot.vision.ready()) {
			...
			OI.errorRumbleDriverMinor_uwu.execute();
			error = true;
			return;
		}
		...
		try {
			...
			OI.errorRumbleDriverMajor_uwu.execute();
			error = true;
			return;
		}
		...
		params.waypoints = new Waypoint[] { new Waypoint(0, 0, Math.PI / 2),
				// The second waypoint has coordinates relative to the first waypoint, which is
				// just the robot's current position
		...
		// We can't call start() on the command as it also requires drivetrain, which
		// would cause this command to be interrupted.
	...
	protected void execute() {
		...
		/*
		 * try { // Try to get the vision values // If this throws and error, the async
		 * trajectory generation is skipped visionAngleOffset =
		 * Robot.vision.getTargetAngleOffset(); visionXOffset =
		 * Robot.vision.getTargetXOffset(); visionYOffset =
		 * Robot.vision.getTargetYOffset(); // Output them if in debug mode
		 * if(Robot.isInDebugMode) { SmartDashboard.putNumber("Auto Align X Offset",
		 * visionXOffset); SmartDashboard.putNumber("Auto Align Y Offset",
		 * visionYOffset); SmartDashboard.putNumber("Auto Align Angle Offset",
		 * visionAngleOffset); }
		 * 
		 * // Check if the async trajectory generation is done if(trajGenFuture != null
		 * && trajGenFuture.isDone()) { try { // If it's done, replace the current
		 * trajectory trajectory = trajGenFuture.get(); // Replace the follower command
		 * and initialize followerCommand = new FollowTrajectory(trajectory);
		 * followerCommand.initialize(); } // Catch a crap ton of exceptions
		 * catch(InterruptedException e) {
		 * RobotLogger.logError("New trajectory generation was interrupted"); }
		 * catch(ExecutionException e) {
		 * RobotLogger.logError("Exception in new trajectory generation: " +
		 * e.getMessage()); } catch(CancellationException e) {
		 * RobotLogger.logError("New trajectory generation was cancelled"); }
		 * 
		 * // Start a new generation thread // Make sure that the target is in sight and
		 * more than 10 inches away if(!Double.isNaN(visionAngleOffset) && visionYOffset
		 * > 10) { // Submit a callable that generates the new trajectory trajGenFuture
		 * = executorService.submit(() -> { params = new TrajectoryParams();
		 * params.waypoints = new Waypoint[] { new WaypointEx(0, 0, Math.PI / 2,
		 * (Robot.drivetrain.getLeftSpeed() + Robot.drivetrain.getRightSpeed()) / 2),
		 * new Waypoint(visionXOffset, visionYOffset, Math.toRadians(-visionAngleOffset)
		 * + Math.PI / 2), }; params.alpha = params.alpha = Math.sqrt(visionXOffset *
		 * visionXOffset + visionYOffset * visionYOffset) * 0.75; params.isTank = true;
		 * params.segmentCount = 100; return new TankDriveTrajectory(RobotMap.specs,
		 * params); }); } // If conditions are not met, set the future to be null to
		 * attempt generation again next loop else { trajGenFuture = null; } } //
		 * Otherwise if the future is null then generate it else if(trajGenFuture ==
		 * null) { if(!Double.isNaN(visionAngleOffset) && visionYOffset > 10) {
		 * trajGenFuture = executorService.submit(() -> { params = new
		 * TrajectoryParams(); params.waypoints = new Waypoint[] { new WaypointEx(0, 0,
		 * Math.PI / 2, (Robot.drivetrain.getLeftSpeed() +
		 * Robot.drivetrain.getRightSpeed()) / 2), new Waypoint(visionXOffset,
		 * visionYOffset, Math.toRadians(-visionAngleOffset) + Math.PI / 2), };
		 * params.alpha = params.alpha = Math.sqrt(visionXOffset * visionXOffset +
		 * visionYOffset * visionYOffset) * 0.75; params.isTank = true;
		 * params.segmentCount = 100; return new TankDriveTrajectory(RobotMap.specs,
		 * params); }); } else { trajGenFuture = null; } } } catch(VisionException e) {
