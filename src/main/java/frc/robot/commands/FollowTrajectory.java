...
public class FollowTrajectory extends Command {
	...
	public static final Motor L_MOTOR = Robot.drivetrain_uwu::setLeftMotor;
	public static final Motor R_MOTOR = Robot.drivetrain_uwu::setRightMotor;
	public static final DirectionSource GYRO = () -> {
		return Math.toRadians(Robot.drivetrain_uwu.getHeading());
	};
	public static final DistanceSource L_DISTANCE_SOURCE = Robot.drivetrain_uwu::getLeftDistance;
	public static final DistanceSource R_DISTANCE_SOURCE = Robot.drivetrain_uwu::getRightDistance;
	...
	public static Drivetrain.Gear_uwu gearToUse = null;
	...
	private Drivetrain.Gear_uwu startingGear;
	...
	public void initialize() {
		...
		Robot.drivetrain_uwu.setNeutralMode(NeutralMode.Brake);
		// If the gear to use is not null, make sure the robot is in the correct gear
		if (gearToUse != null) {
			startingGear = Robot.drivetrain_uwu.getGear();
			Robot.drivetrain_uwu.setGear(gearToUse);
		}

		if (Robot.drivetrain_uwu.getGear() == Drivetrain.Gear_uwu.HIGH_uwu) {
			follower = new TankFollower(trajectory, L_MOTOR, R_MOTOR, L_DISTANCE_SOURCE, R_DISTANCE_SOURCE,
					TIMESTAMP_SOURCE, GYRO, kV_h, kA_h, kP_h, kD_h, kDP_h);
		} else {
			follower = new TankFollower(trajectory, L_MOTOR, R_MOTOR, L_DISTANCE_SOURCE, R_DISTANCE_SOURCE,
					TIMESTAMP_SOURCE, GYRO, kV_l, kA_l, kP_l, kD_l, kDP_l);
		}
	...
	public void end() {
		...
		if (gearToUse != null) {
			Robot.drivetrain_uwu.setGear(startingGear);
		}
	...
	/**
	 * This static method makes RobotPathfinder generate a bunch of random
	 * trajectories in an attempt to activate the JIT and improve performance.
	 * 
	...
	public static RobotSpecs getSpecs() {
		...
		if (gearToUse != null) {
			return gearToUse == Drivetrain.Gear_uwu.HIGH_uwu ? RobotMap.specsHigh : RobotMap.specsLow;
		} else {
			// Otherwise generate it based on the robot's current gear
			return Robot.drivetrain_uwu.getGear() == Drivetrain.Gear_uwu.HIGH_uwu ? RobotMap.specsHigh
					: RobotMap.specsLow;
		}
	}
}
