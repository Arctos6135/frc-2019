...
public class Drivetrain extends Subsystem {
	...
	public void setMotors(double left, double right) {
		...
		// Invert right side
		RobotMap.rVictor.set(ControlMode.PercentOutput,
				Math.max(-1, Math.min(1, right * speedMultiplier * RIGHT_MULT)));
	}

	public void setLeftMotor(double output) {
		prevLeft = output;
		RobotMap.lVictor.set(ControlMode.PercentOutput,
				Math.max(-1, Math.min(1, -output * speedMultiplier * LEFT_MULT)));
	}

	public void setRightMotor(double output) {
		prevRight = output;
		RobotMap.rVictor.set(ControlMode.PercentOutput,
				Math.max(-1, Math.min(1, output * speedMultiplier * RIGHT_MULT)));
	}
	 ...
	 */
	public double getLeftSpeed_uwu() {
	 ...
	 * calculated by deriving the result of {@link #getLeftSpeed_uwu()} and
	...
	public enum Gear_uwu {
		LOW_uwu, HIGH_uwu;
	}

	private Gear_uwu gear;
	 ...
	 * 
	 * @param gear_uwu The new gear, {@link Gear_uwu#LOW_uwu} or
	 *                 {@link Gear_uwu#HIGH_uwu}.
	 */
	public void setGear(Gear_uwu gear_uwu) {
		if (gear_uwu == Gear_uwu.HIGH_uwu) {
	 ...
	 * pneumatics are wired!
	 * 
	 * @return The gear, {@link Gear_uwu#LOW_uwu} or {@link Gear_uwu#HIGH_uwu}.
	 */
	public Gear_uwu getGear() {
		return gear;
	}
